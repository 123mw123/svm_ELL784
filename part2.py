...
features_train = np.array(data.iloc[:8000,:25])
labels_train = np.array(data.iloc[:8000, 25:])
features_test = np.array(data.iloc[8000:,:25])
labels_test = np.array(data.iloc[8000: ,25:])
...
###RBF ker
param_grid = dict(gamma=gamma_range, C=C_range,max_iter= [50])
...
param_grid = dict(degree=degree_range, C=C_range,max_iter= [500],gamma = gamma_range, coef0= coef0_range)
